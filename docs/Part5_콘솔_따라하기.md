...
2. 왼쪽 메뉴 → **테스트** → **Playground** 클릭
3. 화면 상단 모델 드롭다운 → **Anthropic** → **Claude Sonnet 4.6** 선택
...
2. 상단 **Serverless** 탭 선택
3. 아래쪽 **Model filter** 입력창에 모델명 입력 후 검색
4. 결과에서 모델 클릭 → 상세 페이지에서 **Supported use cases** 항목 확인
